...
Because the provider is swappable, so is the price: the deployed demo runs on Claude Haiku at the rates below, and the same code on Gemini Flash's free tier costs nothing. Per-draft token counts and cost are recorded on every `messages` row and shown in the UI, so the figure is measured rather than asserted.
